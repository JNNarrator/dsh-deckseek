...
> 工作文档：跨机器续作用。**2026-09-14 已实施：T1–T7 全部完成，随 0.8.0 发布**。
...
## 实施记录（2026-09-14）——计划与实做的四处偏差

实施时按真实 DOM 与真实字宽做了调整，均记在此处，避免后来人照计划文本找实现：

1. **呼吸点改成"单字呼吸"，放弃了 `· • ● •` 逐帧轮转。** 逐帧轮转需要一个固定宽度（1ch）的窗口把四个字形裁出来，而 `●`（U+25CF）在 CJK 度量的等宽字体里是宽字，会被那格裁掉一半——正是 0.7.4 刚修掉的 `⏺` 半圆问题。改用 dsh-TUI 自己的 reduced-motion 处理：一个 `●` 呼吸（`glyphBreathe` 1.4s）。机制上仍是"状态行前导字形"，但没有引入裁剪风险。

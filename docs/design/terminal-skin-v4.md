...
**T2 的数据边界**：读数只用插件已经持有的东西——分好的轮数、最新一轮的步数、`hasMore`（历史未载完时把轮数标成下限 `N+ 轮`）。四家在这一格都印 token / 花费 / 上下文占用；**0.1.7 修订（2026-09-23）**：宿主自己现在也印了（`StatsPills` / `ContextMeter`，挂在 composer dock，而插件只替换 `conversation.view`），所以这一格的结论从「我们没有那个投影」改为**「那一格是宿主的，宿主已经填了，插件不重复填」**——同屏两份不同源的统计比留白更糟。详见 `compat-0.1.7.md` §11.3。

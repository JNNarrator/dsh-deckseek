 ...
 */
export function turnStructure(policy: WorkDetailPolicy): TurnStructure {
  if (!policy.groupProcess) return 'flat';
...
function isInputNode(node: ChatConversationViewNode): boolean {
 ...
 */
export function hasInterleavedInput(keys: readonly string[], get: (key: string) => ChatConversationViewNode | undefined): boolean {
 ...
 */
export function processExpanded(choice: boolean | undefined, boundary: TurnBoundary, policy: WorkDetailPolicy, interleaved = false): boolean {
  if (choice !== undefined) return choice;
  const structure = turnStructure(policy);
  if (structure === 'flat' || structure === 'open-header') return true;

...
export function splitAnnouncement(text: string, max = 180): string[] {
  const chunks: string[] = [];
  let rest = text.trim();
 ...
 */
export function useLiveAnnounce(text: string): string[] {
  const announcedRef = useRef(0);
  const [chunks, setChunks] = useState<string[]>([]);
  useEffect(() => {
    if (text.length <= announcedRef.current) return;

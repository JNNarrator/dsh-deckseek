...
export function toolIdentity(entry: Pick<ToolActivityEntry, 'block' | 'draft'>) {
  ...
  if (!block) return { name: draft?.name ?? others, raw: draft?.argsRaw ?? '' };
  ...
  if ('kind' in block) return { name: block.call?.name ?? draft?.name ?? others, raw: block.call?.argsRaw ?? draft?.argsRaw ?? '' };
 ...
 */
export function liveToolEntry(flow: readonly ReaderFlowEntry[]): ToolActivityEntry | undefined {
  for (let index = flow.length - 1; index >= 0; index -= 1) {
    const entry = flow[index];
    if (entry?.kind !== 'tool') continue;
    if (entry.block !== undefined && 'kind' in entry.block) continue;
    return entry;
  }
  return undefined;
}

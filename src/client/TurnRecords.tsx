...
import { currentLocale, messageClock, turnProcessLabel, turnTailDetail, turnTailStats, ui, type TurnProcessData, type TurnTailData } from './locale.js';
 ...
 */
export const TurnTailStats = memo(function TurnTailStats({ data, forkAt }: { data: TurnTailData; forkAt?: (seq: number) => void }) {
  ...
  return <div className={css.turnTail} data-reader-anchor data-reader-turn-tail={data.turn ?? ''} title={detail ?? undefined}>
    {stats !== null && <span data-reader-turn-usage>{stats}</span>}
    {stats !== null && ended !== undefined && <span aria-hidden="true">{ui('turnProcess.separator')}</span>}
    {ended !== undefined && <time dateTime={new Date(ended).toISOString()} data-reader-turn-end>{messageClock(ended, currentLocale())}</time>}
    {branchable && <button
      type="button"
      className={css.branch}
      aria-label={ui('branch.action')}
      aria-disabled={unavailable || undefined}
      aria-describedby={unavailable ? branchReasonId : undefined}
      data-reader-branch
      ...
      onClick={unavailable ? undefined : () => { if (data.seq !== undefined) forkAt?.(data.seq); }}
    >
      <BranchGlyph />
    </button>}
    {branchable && unavailable && <span id={branchReasonId} className={css.branchReason}>{ui('branch.unavailable')}</span>}
  </div>;
});

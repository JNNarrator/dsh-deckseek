...
- **消息导航导轨**：右侧导轨列出当前会话中你发送的每条消息——标题超长自动省略、横线分隔、当前消息放大高亮，点击直达对应消息（恢复原对话的导航体验，窄屏自动隐藏）。
- **状态与跟随**：思考卡片紧跟最新并在结束后停在底部；正在思考时显示「大肥鱼正在思考中… 用时」，工具按类别显示阅读中/已阅读、搜索中/已找到等中英文状态；滚动离开底部时底部中央出现 ⬇ 到底部按钮。

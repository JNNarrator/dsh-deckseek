...
- **会话导出**：阅读页工具栏一键把本轮会话导出为 Markdown 文件（用户提问 + 模型回答，按显示顺序，过程保持折叠），文件名自动带时间戳。
- **轮次键盘导航**：Alt+↑ / Alt+↓ 在你的消息之间跳转，与导轨高亮、落点闪烁同一套定位逻辑（输入框内打字时不触发）。
...
- **140 项单元与组件测试**：覆盖消息投影、Markdown 管道、SEP-1865 解析、自适应高度预算、两行流式跟随，以及搜索定位、复制回执、皮肤分片、光标钩子与样式表契约（颜色只取宿主 token、token 引用可解析）等界面交互（happy-dom）。
...
**v0.9.0 · 非官方 DSH 展示与交互增强插件。只改展示与交互视图，不改 Agent 核心执行逻辑、SDK 或模型凭据。**

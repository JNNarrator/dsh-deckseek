...
- **会话导出 Markdown（R4）**：阅读页工具栏新增「导出」——一键把本轮会话下载为 Markdown 文件；与阅读视图同一文本抽取管线（用户提问 + 模型回答，按显示顺序，执行过程保持折叠），同轮多段回答合并为单一标题，文件名自动带时间戳（`deckseek-export-YYYYMMDD-HHmm.md`）
- **轮次键盘导航（R5）**：Alt+↑ / Alt+↓ 在用户消息之间跳转——当前行判定与导轨高亮同一套视口规则（含贴底归属），跳转复用导轨的平滑滚动、落点闪烁与 pending 锁；被回合窗口折叠的消息沿按键方向顺延到最近可落点；输入框 / 查找框内打字不触发
- **测试**：新增导出构建器与键盘导航组件用例（104/104）

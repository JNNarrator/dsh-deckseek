 ...
 */
import { activityPhase, activitySummary, liveToolEntry } from './tool-activity.js';
 ...
 */
const CATEGORY_ORDER = ['terminal', 'write', 'read', 'search', 'web', 'other'];
/**
...
export function turnCounts(flow, lang = currentLocale()) {
    const files = new Set();
    let tools = 0;
    let failed = 0;
    for (const entry of flow) {
        if (entry.kind !== 'tool')
            continue;
        tools += 1;
        if (activityPhase(entry) === 'failed')
            failed += 1;
        const summary = summaryOf(entry, lang);
        if ((summary.category === 'read' || summary.category === 'write') && summary.target !== undefined)
            files.add(summary.target);
    }
...
function summaryOf(entry, lang) {
    const holder = entry.block ?? entry.draft;
    if (holder === undefined)
        return activitySummary(entry, lang);
    let byLang = summaryCache.get(holder);
    if (byLang === undefined) {
        byLang = new Map();
        summaryCache.set(holder, byLang);
    }
 ...
 */
export function collapsedSummary(counts, lang = currentLocale()) {
    const parts = [];
    if (counts.tools > 0)
        parts.push(uiIn(lang, counts.tools === 1 ? 'frame.toolsOne' : 'frame.tools', { count: counts.tools }));
    if (counts.files > 0)
        parts.push(uiIn(lang, counts.files === 1 ? 'frame.filesOne' : 'frame.files', { count: counts.files }));
    if (counts.failed > 0)
        parts.push(uiIn(lang, 'frame.failed', { count: counts.failed }));
 ...
 * total as a floor.
 */
export function railTurns(items) {
    const turns = new Set();
 ...
 * strings existed.
 */
export function frameMeterLabel(turns, steps, more, lang = currentLocale()) {
    if (turns <= 0)
        return null;
    const turnsKey = more ? 'meter.turnsMore' : turns === 1 ? 'meter.turnsOne' : 'meter.turns';
    const parts = [uiIn(lang, turnsKey, { count: turns })];
    if (steps > 0)
        parts.push(uiIn(lang, steps === 1 ? 'meter.stepsOne' : 'meter.steps', { count: steps }));
    return parts.join(' · ');
}
/**
 ...
 */
export function activityRanks(flow, lang = currentLocale()) {
...
export function dominantCategory(ranks) {
 ...
 */
export function activityPhrase(flow, lang = currentLocale()) {
    ...
    if (second === undefined)
        return first;
    if (labels.length === 2)
        return uiIn(lang, 'frame.activity.join', { first, second });
 ...
 */
export function liveFramePhase(flow, lang = currentLocale()) {
 ...
 */
export function liveFrameLabel(flow, lang = currentLocale()) {
    const live = liveFramePhase(flow, lang);
    if (live === null)
        return null;
    return uiIn(lang, `frame.${live.phase}.${live.category}`);
}
//# sourceMappingURL=frame-meter.js.map
...
function ModelFacingContent({ content, t }) {
    return (_jsx(_Fragment, { children: contentRuns(content).map((run, index) => ('text' in run
            ...
            : (_jsx(JsonBlock, { label: t ? t('message.unknownBlock') : unknownBlockLabel(), payload: run.block, truncatedLabel: t ? total => t('json.truncated', { total }) : truncatedJsonLabel }, index)))) }));
}
/**

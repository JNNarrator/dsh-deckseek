...
function GroupStatus({ group, sessionId, useChat, useSessionPendingInteraction, motion, variant }) {
    ...
    if (kind === 'delving')
        return null;
    return _jsx(StatusText, { text: text, ariaText: ariaText, motion: motion, shimmer: busy, swapKey: kind });
}
...
const TurnGroup = memo(function TurnGroup({ group, motion, pinnedKeys, selectedProcessKeys, ...props }) {
    ...
    const headerStatus = _jsx(GroupStatus, { group: group, sessionId: props.sessionId, useChat: props.useChat, useSessionPendingInteraction: props.useSessionPendingInteraction, motion: motion, variant: "header" });

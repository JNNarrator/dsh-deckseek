...
export const SearchPanel = memo(function SearchPanel({ root, index, onClose }) {
    ...
    return (_jsxs("div", { ref: row, className: css.searchRow, "data-reader-search": true, role: "search", children: [_jsx("input", { ref: input, className: css.searchInput, value: query, onChange: event => { setQuery(event.target.value); setCursor(0); }, placeholder: ui('reader.searchPlaceholder'), onKeyDown: event => {
                    if (event.key === 'Enter') {
                ...
                } }), _jsx("span", { className: css.searchMeta, role: "status", children: query.trim() ? (matches.length === 0 ? ui('reader.searchNoMatches') : `${(cursor % matches.length) + 1} / ${matches.length}`) : '' }), _jsx("button", { type: "button", className: css.textButton, disabled: matches.length === 0, title: ui('reader.searchPrevTitle'), onClick: () => step(-1), children: "\u2191" }), _jsx("button", { type: "button", className: css.textButton, disabled: matches.length === 0, title: ui('reader.searchNextTitle'), onClick: () => step(1), children: "\u2193" }), _jsx("button", { type: "button", className: css.textButton, onClick: onClose, children: "\u00D7" })] }));
});

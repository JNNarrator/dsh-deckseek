...
export function pickPreviewText(data) {
    if (isTextLike(data))
        return data;
...
function fieldValue(value, arrayItems) {
    ...
    if (Array.isArray(value))
        return `[…]${arrayItems(value.length)}`;
    return '{…}';
}
...
export function summarizeFields(data, arrayItems = count => `${count} 项`) {
    if (typeof data === 'string' || data === null || data === undefined)
        return '';
    if (typeof data !== 'object')
        return '';
    if (Array.isArray(data))
        return `${ui('unknown.arrayOf')}${arrayItems(data.length)}`;
    const entries = Object.entries(data);
    if (entries.length === 0)
        return ui('unknown.emptyObject');
...
//# sourceMappingURL=unknown-record.js.map
...
export function toolIdentity(entry) {
    ...
    const others = uiIn(currentLocale(), 'tool.others');
    if (!block)
        return { name: draft?.name ?? others, raw: draft?.argsRaw ?? '' };
    // A settled result carries the frozen call; an in-flight one carries its own
    // head. 0.1.7's preparing stage has no argument text at all, so it falls back
    // to whatever the streaming draft has received so far.
    if ('kind' in block)
        return { name: block.call?.name ?? draft?.name ?? others, raw: block.call?.argsRaw ?? draft?.argsRaw ?? '' };
    return { name: block.name, raw: 'argsRaw' in block ? block.argsRaw : draft?.argsRaw ?? '' };
}
...
export function toolFailureFacts(block, lang = currentLocale()) {
 ...
 */
export function toolFailureTitle(block, lang = currentLocale()) {
 ...
 */
export function liveToolEntry(flow) {
    for (let index = flow.length - 1; index >= 0; index -= 1) {
        const entry = flow[index];
        if (entry?.kind !== 'tool')
            continue;
        if (entry.block !== undefined && 'kind' in entry.block)
            continue;
        return entry;
    }
    return undefined;

...
export const TurnRail = memo(function TurnRail({ root, items }) {
    ...
    return (_jsxs("nav", { ref: nav, className: css.rail, "aria-label": ui('rail.label'), children: [_jsx("ul", { className: css.railList, children: items.map(item => (_jsx("li", { className: css.railRow, children: _jsx("button", { type: "button", className: css.railItem, "data-active": item.key === activeKey || undefined, "aria-label": tipOf(item), onClick: () => jump(item), onMouseEnter: event => showTip(item.key, event.currentTarget), onMouseLeave: () => setTip(null), children: _jsx("span", { className: css.railDash, "aria-hidden": true }) }) }, item.key))) }), tip && (() => {

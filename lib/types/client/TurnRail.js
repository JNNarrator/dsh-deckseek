...
export const TurnRail = memo(function TurnRail({ root, items }) {
    ...
    useEffect(() => {
        const onKey = (event) => {
            ...
            if (source && (source.isContentEditable || source.tagName === 'INPUT' || source.tagName === 'TEXTAREA' || source.tagName === 'SELECT'))
                return;
            const content = root.current;
            ...
            if (current === -1 && scroller.scrollHeight - scroller.scrollTop - scroller.clientHeight < 96)
                current = items.length - 1;
            const step = event.key === 'ArrowUp' ? -1 : 1;
            for (let index = current + step; index >= 0 && index < items.length; index += step) {
                if (!anchors.get(items[index].key))
                    continue;

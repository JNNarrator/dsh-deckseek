...
export function ReasoningCard({ children, step, active, history = false, motion, selected, onRead }) {
    ...
    // Settling to the end is only for cards that were live-followed this page
    // life; a freshly mounted historical card must stay put (no scroll on refresh).
    ...
    useLayoutEffect(() => {
        ...
        const host = port?.closest('[data-conversation-scroll]');
        if (!port || !host)
            return;
        const fit = () => {
            ...
            const heading = port.parentElement?.querySelector('[data-reader-reasoning-heading]')?.offsetHeight || 30;
            const footer = port.parentElement?.querySelector('[data-ud-check=reasoning-controls]')?.offsetHeight || 38;
    ...
    useLayoutEffect(() => {
        ...
        let cachedPreviewHeight = null;
        let cachedPreviewWidth = null;
        const previewHeight = () => {
        ...
        const measure = (recordHeight = true) => {
            ...
            if (recordHeight && !resize.current)
        ...
        const schedule = () => {
            if (!canFollow() || frame || timer !== undefined)
                return;
            follow();
            if (tail() - paintedOffset() < 1)
                return;
        ...
        const start = () => {
            ...
            const tick = (now) => {
                ...
                measure();
                if (now - began < REASON_STEP || Math.abs(lastPainted - target) > .05)
        ...
        const onSelection = () => { if (hasSelection())
            pause(); };
        const onVisibility = () => {
            cancel();
            manual();
    ...
    // Historical cards (never live on this page life) never settle — a refresh
    // must not send old thinking scrolling to its end.
    useLayoutEffect(() => {
        if (allowed)
    ...
    useLayoutEffect(() => {
        ...
        if (!port || !text)
            return;
        if (active) {
            settledEnd.current = false;
            return;
        }
        if (!everLive.current || !followingRef.current || settledEnd.current)
            return;
    ...
    useLayoutEffect(() => {
        ...
        if (changed && !selected) {
            ...
            if (card && host) {
                ...
                if (Math.abs(delta) > 1)
                    host.scrollTo({ top: Math.max(0, host.scrollTop + delta), behavior: motion ? 'smooth' : 'instant' });
            }
        }
    ...
    return _jsxs("div", { className: css.reasonCard, "data-reader-reasoning-card": true, "data-reader-anchor": true, "data-expanded": expanded, "data-history": history || undefined, "data-following": allowed, "data-overflow": overflow, "data-ud-motion": "reader-reasoning-size", children: [_jsxs("div", { className: css.reasonHeading, "data-reader-reasoning-heading": true, "data-ud-check": "reasoning-identity", children: [_jsx("span", { className: css.reasonLabel, "data-reader-reasoning-label": true, children: ui('reasoning.label') }), _jsx("span", { children: ui('reasoning.step', { step }) })] }), _jsx("div", { ref: viewport, id: controls, className: css.reasonViewport, "data-reader-reasoning-scroll": true, "data-edges": edges, "data-ud-motion": "reader-reasoning-scroll", role: "region", "aria-label": overflow ? ui('reasoning.scrollAria', { step }) : ui('reasoning.regionAria', { step }), tabIndex: overflow ? 0 : undefined, onPointerDown: pause, onFocus: pause, children: _jsx("div", { ref: track, className: css.reasonTrack, "data-reader-reasoning-track": true, children: _jsx("div", { ref: content, className: css.reasonText, "data-reader-reasoning-text": true, children: children }) }) }), (overflow || expanded || history) && _jsxs("div", { className: css.reasonFooter, "data-ud-check": "reasoning-controls", children: [active && motion ? _jsxs("button", { type: "button", className: css.reasonAction, disabled: selected, "aria-controls": controls, "aria-label": following ? ui('reasoning.pauseFollowAria') : ui('reasoning.resumeFollowAria'), title: selected ? ui('reasoning.followHint') : undefined, onClick: () => { if (following)
                            pause();
                        else {
                            onRead();
                            setFollowing(true);
                        } }, children: [_jsx("svg", { width: "12", height: "12", viewBox: "0 0 16 16", "aria-hidden": "true", children: following ? _jsx("path", { d: "M5.5 4v8m5-8v8" }) : _jsx("path", { d: "M8 3v10m-4-4 4 4 4-4" }) }), following ? ui('reasoning.pauseFollow') : ui('reasoning.resumeFollow')] }) : expanded ? _jsx("span", { className: css.reasonCaption, children: ui('reasoning.manual') }) : null, _jsxs("button", { type: "button", className: css.reasonAction, "aria-expanded": expanded, "aria-controls": controls, "aria-label": expanded ? ui('reasoning.collapseFullAria') : ui('reasoning.expandFullAria'), onClick: toggleReading, children: [expanded ? ui('reasoning.collapse') : ui('reasoning.expandRead'), _jsx("svg", { width: "12", height: "12", viewBox: "0 0 16 16", "aria-hidden": "true", children: expanded ? _jsx("path", { d: "m4 10 4-4 4 4" }) : _jsx("path", { d: "m4 6 4 4 4-4" }) })] })] })] });
}

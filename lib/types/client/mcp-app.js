...
export function getCssTokens(theme) {
 ...
 */
export function ensureHtmlDocument(rawHtml, initialTheme = 'light', lang = 'zh-CN') {
 ...
 */
export function isMcpAppCodeBlock(lang, meta, value) {
 ...
 */
export function extractMcpAppTitle(meta, value) {
    if (meta) {
 ...
 */
export function extractMcpAppHeight(meta) {
    if (!meta)
        return undefined;
 ...
 */
export function formatReceiptPrompt(params, title) {
    ...
    if (keys.length === 1 && typeof params[keys[0]] === 'string') {
        return uiIn(lang, 'mcp.receiptPrompt', { name: title ?? uiIn(lang, 'mcp.receiptFallback'), key: keys[0], value: params[keys[0]] });
    }
 ...
 */
export function setReactInputValue(textarea, value) {
    try {

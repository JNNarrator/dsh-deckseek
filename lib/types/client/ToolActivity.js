...
import { activityPhase, activitySummary, diffStat, executionFacts, objectValue, toolFailureFacts, toolFailureText, toolFailureTitle, toolStateLabel } from './tool-activity.js';
...
const ICONS = { write: EditGlyph, read: BrowseGlyph, terminal: TerminalGlyph, search: SearchGlyph, web: SearchGlyph, other: ToolGlyph };
...
export const ToolActivity = memo(function ToolActivityView({ entry, motion, turnClosed, onRead, depth = 0, cwd, ...render }) {
...
export function ToolMedia({ block, depth = 0, failureNote, ...render }) {
    if (depth > 6)
        return null;
    ...
    return _jsxs(_Fragment, { children: [failed && _jsx(FailureCard, { title: toolFailureTitle(block), message: toolFailureFacts(block) ?? undefined, detail: toolFailureText(block) ?? undefined, note: failureNote, raw: 'kind' in block ? { content: block.content, isError: block.isError, meta: block.meta } : undefined }), visible.length > 0 && _jsx(Blocks, { ...render, blocks: visible, source: "tool" }), block.subCalls.map(child => _jsx(ToolMedia, { ...render, block: child, depth: depth + 1 }, child.callId))] });
}

...
function renderCode(node, key, context) {
    ...
    if (isMcpAppCodeBlock(node.lang, node.meta, node.value)) {
        ...
        return _jsx(McpAppFrame, { html: node.value, title: title, initialHeight: initialHeight }, key);
    }
...
const TableBlock = memo(function TableBlock({ node, wide, streaming, context, labels }) {
    ...
    return (_jsxs("div", { className: css.tableWrap, children: [_jsx("div", { className: clsx(css.tableScroll, wide ? 'md-table-wide' : css.tableFill), tabIndex: wide ? 0 : undefined, children: _jsxs("table", { children: [headRow !== undefined && _jsx("thead", { children: renderTableRow(headRow, 'th', align, 0, context) }), bodyRows.length > 0 && (_jsx("tbody", { children: bodyRows.map((row, index) => renderTableRow(row, 'td', align, index + 1, context)) }))] }) }), !streaming && csv !== '' && (_jsx("button", { type: "button", className: css.tableCopy, title: labels.copyCsvLabel, "aria-label": labels.copyCsvLabel, onClick: async () => {
                    const accepted = await writeClipboard(csv);
                    setCopied(accepted);
                    if (timer.current)
                        clearTimeout(timer.current);
                    timer.current = setTimeout(() => setCopied(false), 2000);
                }, children: copied ? labels.copiedCsvLabel : labels.copyCsvLabel }))] }));
});
function renderTable(node, key, context) {
    ...
    return (_jsx(TableBlock, { node: node, wide: wide, streaming: context.streaming, context: context, labels: {
